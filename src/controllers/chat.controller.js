...
export async function chatController(req, res, next) {
  ...
  try {
    ...
    const reservation = await reserveBudget(req.gatewayKey.id, estimatedCost);
    ...
    const actualCost = calculateCost(result.inputTokens, result.outputTokens);

    await reconcileBudget(req.gatewayKey.id, reservedCost, actualCost);

    reservedCost = 0;

   await prisma.usageLog.create({
    data: {
        keyId: req.gatewayKey.id,
    ...
    if (reservedCost > 0 && req.gatewayKey?.id) {
      try {
        await releaseBudget(req.gatewayKey.id, reservedCost);
      } catch (releaseError) {
